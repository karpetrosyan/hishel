...
def test_invalid_key_symbol():
    ...
    with pytest.raises(
        ParseError,
        match=r"The character ''\\x12'' is not permitted in the directive name.",
    ):
        parse_cache_control(header)
...
def test_invalid_symbol_in_unquoted():
    ...
    with pytest.raises(
        ParseError,
        match=r"The character ''\\x12'' is not permitted for the unquoted values.",
    ):
        parse_cache_control(header)
...
def test_invalid_symbol_in_quoted():
    ...
    with pytest.raises(
        ParseError,
        match=r"The character ''\\x12'' is not permitted for the quoted values.",
    ):
        parse_cache_control(header)
...
def test_time_field_invalid_int():
    ...
    with pytest.raises(
        ValidationError,
        match="The argument 'max_age' should be an integer, but got ''123t1''.",
    ):
        parse_cache_control(header)
...
def test_boolean_fields_with_value():
    ...
    with pytest.raises(
        ValidationError,
        match="The directive 'no_store' should have no value, but it does.",
    ):
        parse_cache_control(header)
