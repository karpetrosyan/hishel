...
class S3Manager:
    def __init__(self, client: tp.Any, bucket_name: str, check_ttl_every: int | float, is_binary: bool = False):
        self._client = client
    ...
    def write_to(self, path: str, data: bytes | str, only_metadata: bool = False) -> None:
        path = "hishel-" + path
    ...
    def read_from(self, path: str) -> bytes | str:
        path = "hishel-" + path
        response = self._client.get_object(
            Bucket=self._bucket_name,
            Key=path,
...
class AsyncS3Manager:  # pragma: no cover
    ...
    async def write_to(self, path: str, data: bytes | str, only_metadata: bool = False) -> None:
        return await to_thread.run_sync(self._sync_manager.write_to, path, data, only_metadata)

    async def read_from(self, path: str) -> bytes | str:
